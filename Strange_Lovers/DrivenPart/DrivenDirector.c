...
float DrivenDirector_calc_turn_value(DrivenDirector *self,float target_brightness, float target_curvature,float target_runner_angle,ControllerWeight use_controller,MovementDirection movementDirection)
{
	...
	logSend(0,0,DirectionEncoder_get_direction(&directionEncoder),DistanceEncoder_get_total_distance(&distanceEncoder),SonarEncoder_get_distance_obstacle(&sonorEncoder),CoordinateEncoder_get_yCoo(&coordinateEncoder),0,0);
